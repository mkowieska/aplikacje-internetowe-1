const urlAktualnaPogoda = 'https:/weather';
const urlPrognozaPogoda = 'https:/forecast';
...
function pobierzAktualnaPogode(miasto) {
    ...
    xhr.open('GET', `${urlAktualnaPogoda}?q=${miasto}&appid=${klucz}&units=metric`);
    xhr.onload = function () {
        if (xhr.status === 200) {
...
function pobierzPrognozePogody(miasto) {
    fetch(`${urlPrognozaPogoda}?q=${miasto}&appid=${klucz}&units=metric`)
        .then(odpowiedz => {
            if (!odpowiedz.ok) {
